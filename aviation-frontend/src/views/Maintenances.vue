<template>
  <div class="app-container">
    ...
    <div style="margin-bottom: 20px; display: flex; gap: 10px;">
      ...
      <el-button type="success" @click="openCreateDialog">新建维修工单</el-button>
    </div>
  ...
  <el-table-column prop="component_no" label="维修部件编号" width="150" />
      ...
      <el-table-column label="状态" width="100">
        <template #default="scope">
          <el-tag v-if="scope.row.result === 'pending'" type="warning">维修中</el-tag>
          <el-tag v-else-if="scope.row.result === 'passed'" type="success">已通过</el-tag>
          <el-tag v-else-if="scope.row.result === 'failed'" type="danger">未通过</el-tag>
          <el-tag v-else-if="scope.row.result === 'scrapped'" type="info">已报废</el-tag>
    ...
    <el-dialog title="新建维修工单" v-model="createVisible" width="500px">
      <el-form :model="createForm" label-width="100px">
        ...
        <el-form-item label="维修类型" required>
          <el-select v-model="createForm.maintenance_type" style="width: 100%">
            <el-option label="常规检查 (routine)" value="routine" />
            <el-option label="故障维修 (repair)" value="repair" />
            <el-option label="深度大修 (overhaul)" value="overhaul" />
          </el-select>
        </el-form-item>
        <el-form-item label="负责人员" required>
          <el-select v-model="createForm.technician_id" placeholder="请选择维修人员" style="width: 100%">
            <el-option 
              ...
              :key="op.operator_id" 
              :label="op.operator_name + ' (' + op.role + ')'" 
