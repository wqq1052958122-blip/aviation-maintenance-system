...
FROM Component
WHERE component_no IN ('ENG-004', 'ENG-005', 'ENG-006', 'NAV-002', 'HYD-001')
...
SELECT c.component_id, a.aircraft_id, p.position_id, p.position_code,
       '2025-06-10 09:00:00', NULL, 'illegal installation for retired component', NULL, 1, NULL
FROM Component c
JOIN Aircraft a ON a.aircraft_no = 'AC-1008'
JOIN AircraftInstallPosition p ON p.aircraft_id = a.aircraft_id AND p.position_code = 'hydraulic system bay'
...
INSERT INTO InstallationRecord (component_id, aircraft_id, position_id, install_position, install_time, uninstall_time, install_reason, uninstall_reason, operator_id, uninstall_operator_id)
SELECT c.component_id, a.aircraft_id, p.position_id, p.position_code,
       '2025-06-10 10:00:00', NULL, 'illegal duplicate installation', NULL, 1, NULL
FROM Component c
JOIN Aircraft a ON a.aircraft_no = 'AC-1002'
JOIN AircraftInstallPosition p ON p.aircraft_id = a.aircraft_id AND p.position_code = 'navigation bay'
WHERE c.component_no = 'NAV-002';

-- 测试3：同一飞机同一位置不能同时安装多个部件。预期：Aircraft position already has an active component.
-- ENG-005 是 available，可安装；但 AC-1001 的 left engine position 已被 ENG-002 占用。
INSERT INTO InstallationRecord (component_id, aircraft_id, position_id, install_position, install_time, uninstall_time, install_reason, uninstall_reason, operator_id, uninstall_operator_id)
SELECT c.component_id, a.aircraft_id, p.position_id, p.position_code,
       '2025-06-10 11:00:00', NULL, 'illegal position conflict test', NULL, 1, NULL
FROM Component c
JOIN Aircraft a ON a.aircraft_no = 'AC-1001'
JOIN AircraftInstallPosition p ON p.aircraft_id = a.aircraft_id AND p.position_code = 'left engine position'
WHERE c.component_no = 'ENG-005';
...
CALL sp_retire_component('NAV-002', '2025-06-12 10:00:00', 'illegal retirement test', 3, 'this should fail because NAV-002 is still installed');
...
-- LDG-002 是 landing_gear 类且 available，battery bay 只允许 battery 类。
INSERT INTO InstallationRecord (component_id, aircraft_id, position_id, install_position, install_time, uninstall_time, install_reason, uninstall_reason, operator_id, uninstall_operator_id)
SELECT c.component_id, a.aircraft_id, p.position_id, p.position_code,
       '2025-06-22 09:00:00', NULL, 'category mismatch test', NULL, 1, NULL
FROM Component c
JOIN Aircraft a ON a.aircraft_no = 'AC-1001'
